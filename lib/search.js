...
function mixinSearch(PostgreSQL) {
    PostgreSQL.prototype.getJoinQuery = function(model, where) {
        ...
        for (let key in where) {
            if (key.indexOf('__') >= 0) {
                // compose key
                const Model = this._models[model].model
    ...
    PostgreSQL.prototype.buildWhere = function(model, where) {
        ...
        let searchObj = getSearchQuery(where)
        let [joinQuery, whereQuery] = this.getJoinQuery(model, where)
    ...
    PostgreSQL.prototype._buildWhere = function(model, where) {
      ...
      for (let key in where) {
        ...
        let columnName = self.escapeName(model.toLowerCase()) + '.' + self.columnEscaped(model, key);
